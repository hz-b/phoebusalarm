...
class TestOrdering(unittest.TestCase):
    def test_string_int(self):
        alarm_1 = AlarmPV("test:pv0", sortKey="-1")
        alarm00 = AlarmPV("test:pv0", sortKey="")
...
class TestAlarmNode(unittest.TestCase):
    ...
    def test_display_abs_path(self):
        ...
        self.alarmNode.add_display("title", "/path/to/display.bob", macros)
        self.assertEqual(
    ...
    def test_display_url(self):
        ...
        self.alarmNode.add_display("title", "file:///path/to/display.bob", macros)
        self.assertEqual(
            self.alarmNode.displays[0].details,
    ...
    def test_display_url_with_query(self):
        ...
        self.alarmNode.add_display(
            "title", "file:///path/to/display.bob?DEV=7&B=some+other+thing", macros
        )
        self.assertEqual(
            self.alarmNode.displays[0].details,
...
class TestAlarmNodeAlh(unittest.TestCase):
    ...
    def test_action(self):
        expectation = ["GROUP parent Name", "$SEVRCOMMAND UP_ANY do_something.sh blubb"]
...
class TestAlarmPV(unittest.TestCase):
    ...
    def test_delay(self):
        ...
        self.assertEqual(len(delayEle), 1, "there must be one delay element")
        self.assertEqual(delayEle[0].text, "10", "delay element has incorrect text")
    ...
    def test_count(self):
        ...
        self.assertEqual(
            len(countEle), 0, "there should be no count element without delay"
        )
        self.alarmPV.delay = 10
        xml = self.alarmPV.get_xml_element()
        countEle = xml.findall("count")
        self.assertEqual(
            len(countEle), 1, "there should be one count element with delay"
        )
        self.assertEqual(countEle[0].text, "5", "count element has incorrect text")
